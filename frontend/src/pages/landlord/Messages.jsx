    ...
    <div className="flex flex-col h-full">
      <div className="border-b border-border px-4 py-2.5 shrink-0 flex items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="font-semibold truncate">{conversation.applicant_name}</p>
          <div className="flex items-center gap-2 text-sm text-muted-foreground mt-0.5">
            {conversation.property_title && (
              <Link to={`/objekte/${conversation.property_id}`} className="flex items-center gap-1 hover:text-primary min-w-0">
                <Building2 className="h-3.5 w-3.5 shrink-0" /> <span className="truncate">{conversation.property_title}</span>
              </Link>
            )}
          </div>
        </div>
        {onBack && (
          <button onClick={onBack} className="lg:hidden shrink-0 p-2.5 -m-1 rounded-md hover:bg-secondary text-muted-foreground"
            aria-label="Zurück zur Übersicht" data-testid="thread-back">
